...
function buildCostPlannerWorkspaceHtml(cfg) {
  ...
  const styles =
    ...
    '.' + p + '-row-actions{white-space:nowrap;text-align:right;}' +
  ...
  const script =
    ...
    'function setRowEditing(tr,on){if(!CFG.paidOffStatus||!tr)return;var editing=!!on;var editBtn=tr.querySelector("."+P+"-debt-edit");if(editing)tr.classList.add(P+"-row-editing");else tr.classList.remove(P+"-row-editing");if(editBtn)editBtn.textContent=editing?"Done":"Edit";var labInp=tr.querySelector("."+P+"-debt-label-inp");var labTxt=tr.querySelector("."+P+"-debt-label-txt");var origInp=tr.querySelector("."+P+"-orig-inp");var origTxt=tr.querySelector("."+P+"-orig-txt");var balInp=tr.querySelector("."+P+"-balance");var balTxt=tr.querySelector("."+P+"-balance-txt");if(labInp)labInp.hidden=!editing;if(labTxt)labTxt.hidden=editing;if(origInp)origInp.hidden=!editing;if(origTxt)origTxt.hidden=editing;if(balInp&&balTxt){var logLocked=balInp.readOnly;if(editing&&!logLocked){balInp.hidden=false;balTxt.hidden=true;}else{balInp.hidden=true;balTxt.hidden=false;}}if(editing){try{var focusEl=labInp||origInp;focusEl&&focusEl.focus();}catch(e){}}else syncRowView(tr);}' +
    ...
    'function debtActionsCell(isNew){if(!CFG.paidOffStatus)return "<td><button type=\\"button\\" class=\\""+P+"-debt-delete\\" title=\\"Delete row\\" aria-label=\\"Delete row\\">Delete</button></td>";return "<td class=\\""+P+"-row-actions\\"><button type=\\"button\\" class=\\""+P+"-debt-edit\\" title=\\"Edit category\\" aria-label=\\"Edit category\\">"+(isNew?"Done":"Edit")+"</button><button type=\\"button\\" class=\\""+P+"-debt-delete\\" title=\\"Delete row\\" aria-label=\\"Delete row\\">Delete</button></td>";}' +
