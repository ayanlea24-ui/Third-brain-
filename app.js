...
function buildProjectedDateWorkspaceHtml() {
  ...
  const clientJs =
    ...
    'function todayNoon(){var t=new Date();t.setHours(12,0,0,0);return t;}' +
    'function daysFromToday(ds){var t=todayNoon();var d=new Date(ds+"T12:00:00");return Math.round((d-t)/86400000);}' +
    'function daysText(n){if(n===0)return "Today";if(n===1)return "1 day left";if(n>1)return n+" days left";if(n===-1)return "1 day ago";return Math.abs(n)+" days ago";}' +
    ...
    'function renderTable(){var tb=document.getElementById("pd-list-tbody");if(!tb)return;var list=dates.slice().sort(function(a,b){return String(a.date).localeCompare(String(b.date));});tb.innerHTML="";if(!list.length){tb.innerHTML="<tr><td colspan=\\"4\\">No dates yet.</td></tr>";return;}list.forEach(function(row){var tr=document.createElement("tr");var days=daysFromToday(row.date);var daysCls=days===0?"pd-days-today":days>0?"pd-days-future":"pd-days-past";tr.innerHTML="<td>"+esc(row.date)+"</td><td class=\\""+daysCls+"\\">"+esc(daysText(days))+"</td><td>"+esc(row.label||"")+"</td><td><button type=\\"button\\" class=\\"pd-del\\" data-id=\\""+esc(row.id)+"\\">Delete</button></td>";tb.appendChild(tr);});}' +
    'function renderTimeline(){var host=document.getElementById("pd-timeline");if(!host)return;var today=todayNoon();var todayStr=today.toISOString().slice(0,10);var list=dates.slice().sort(function(a,b){return String(a.date).localeCompare(String(b.date));});var all=list.map(function(r){return r.date;});if(all.indexOf(todayStr)<0){var ins=all.length;for(var i=0;i<all.length;i++){if(all[i]>todayStr){ins=i;break;}}all.splice(ins,0,todayStr);}if(!list.length&&all.length<=1){host.innerHTML="<p class=\\"pd-empty\\">Add a date and its significance — your calendar timeline appears here.</p>";return;}var html="";all.forEach(function(ds,idx){var d=new Date(ds+"T12:00:00");var dateLbl=d.toLocaleDateString("en-CA",{month:"short",day:"numeric",year:"numeric"});var dayLbl=d.toLocaleDateString("en-CA",{weekday:"long"});var isToday=ds===todayStr;var rows=list.filter(function(r){return r.date===ds;});var isPast=ds<todayStr;var days=daysFromToday(ds);var daysLbl=daysText(days);var dotCls="pd-dot";if(isToday)dotCls+=" pd-dot-today";else if(rows.length)dotCls+=isPast?" pd-dot-past":" pd-dot-future";else dotCls+=" pd-dot-muted";var itemCls="pd-item";if(isToday)itemCls+=" pd-item-today";html+="<div class=\\""+itemCls+"\\">";html+="<div class=\\"pd-item-left\\"><span class=\\"pd-date\\">"+esc(dateLbl)+"</span><span class=\\"pd-day\\">"+esc(dayLbl)+"</span>";if(!isToday||rows.length)html+="<span class=\\"pd-days\\">"+esc(daysLbl)+"</span>";html+="</div>";html+="<div class=\\"pd-rail\\"><span class=\\""+dotCls+"\\"></span>";if(idx<all.length-1)html+="<span class=\\"pd-line\\"></span>";html+="</div><div class=\\"pd-body\\">";if(isToday)html+="<p class=\\"pd-you\\">You are here</p>";rows.forEach(function(r){html+="<p class=\\"pd-label\\">"+esc(r.label||"Important date")+"</p>";});if(isToday&&!rows.length)html+="<p class=\\"pd-muted\\">Today</p>";html+="</div></div>";});host.innerHTML=html;}' +
  ...
  return (
    ...
    '.pd-days{display:block;font-size:10px;font-weight:700;color:#6366f1;margin-top:4px}' +
    '.pd-days-today{color:#0d9488;font-weight:700}' +
    '.pd-days-future{color:#6366f1;font-weight:700}' +
    '.pd-days-past{color:#64748b}' +
    ...
    '<div class="pd-panel">' +
    '<h2>All dates</h2>' +
    '<table class="pd-table" aria-label="Projected dates"><thead><tr><th>Date</th><th>From today</th><th>Significance</th><th></th></tr></thead>' +
    '<tbody id="pd-list-tbody"><tr><td colspan="4">Loading…</td></tr></tbody></table>' +
    '</div></div>' +
    '<div class="pd-right pd-panel">' +
    '<h2>Calendar timeline</h2>' +
    '<p class="pd-lead">Chronological view — date, weekday, and days from today on the left; significance on the right.</p>' +
