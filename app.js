...
function buildCostPlannerWorkspaceHtml(cfg) {
  ...
  const script =
    ...
    'elDebtTb.addEventListener("input",function(ev){var t=ev.target;if(!t)return;var tr=t.closest("tr");if(t.classList.contains(P+"-orig-inp")&&tr){var applied=sumAllocationsForId(tr.getAttribute("data-row-id"));var orig=parseNum(t.value);tr.setAttribute("data-target-amt",String(orig));var balInp=tr.querySelector("."+P+"-balance");if(balInp)balInp.value=String(Math.max(0,Math.round((orig-applied)*100)/100));syncRowPaySummary(tr);syncRowPaidOff(tr);recalc();return;}if(t.classList.contains(P+"-balance")&&CFG.paidOffStatus&&tr){var applied2=sumAllocationsForId(tr.getAttribute("data-row-id"));var bal=parseNum(t.value);var origInp2=tr.querySelector("."+P+"-orig-inp");var nextOrig=Math.round((bal+applied2)*100)/100;if(origInp2)origInp2.value=String(nextOrig);tr.setAttribute("data-target-amt",String(nextOrig));syncRowPaySummary(tr);syncRowPaidOff(tr);}if(t.classList.contains(P+"-debt-label-inp")||t.classList.contains(P+"-balance"))recalc();});' +
    'elDebtTb.addEventListener("change",function(ev){var t=ev.target;if(t&&(t.classList.contains(P+"-balance")||t.classList.contains(P+"-orig-inp")))recalc();});' +
