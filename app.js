...
function buildCostPlannerWorkspaceHtml(cfg) {
  ...
  const script =
    ...
    'function refreshRowEditUi(tr){if(!CFG.paidOffStatus||!tr||!tr.classList.contains(P+"-row-editing"))return;var delBtn=tr.querySelector("."+P+"-debt-delete");if(delBtn)delBtn.hidden=false;syncRemainingDisplay(tr);var balInp=tr.querySelector("."+P+"-balance");var balTxt=tr.querySelector("."+P+"-balance-txt");if(balInp&&balTxt){balInp.hidden=true;balTxt.hidden=false;}}' +
    ...
    'elDebtTb.addEventListener("input",function(ev){var t=ev.target;if(!t)return;var tr=t.closest("tr");if(t.classList.contains(P+"-orig-inp")&&tr){var orig=parseNum(t.value);tr.setAttribute("data-target-amt",String(orig));syncRemainingDisplay(tr);var id=tr.getAttribute("data-row-id");var paidEl=tr.querySelector("."+P+"-paid-amt");if(paidEl)paidEl.textContent=fmt(sumAllocationsForId(id));var items=readDebtsFromDom();var total=sumDebtItems(items);elTotal.textContent=fmt(total);save(readState());return;}if(t.classList.contains(P+"-debt-label-inp"))recalc();});' +
